...
The potentiostat is responsible for polarizing the electrochemical cell to a voltage V~CELL~and reading the current flowing through I~CELL~. In addition, the ADC of the microcontroller will be used to read the real voltage that we are applying and to calculate V~CELL~and I~CELL~.

### Microcontroller configuration

| Pin  | Alias |         Type         | Description                                                  |
| :--: | :---: | :------------------: | :----------------------------------------------------------- |
| PA0  | VREF  |     Analog input     | Absolute voltage of the *Reference Electrode (RE)* V~REF~used for the V~CELL~ measurement. |
| PA1  | ICELL |     Analog input     | TIA output voltage used for I~CELL~ measurement.             |
...
Sampling rate is set to 100ms, and V~CELL~ is fixed at 0.5V. The results obtained are shown below. It is observed how current is around 2,3E-5 A. 
